...
import numpy as np
import pandas as pd
from project.models import init_model, Model
from project.enities import ModelsParams, ClassifierParams
...
def test_type_model(dict_class_params: dict, abs_path: str):
    params = ModelsParams(
        scaling_path=os.path.join(abs_path, "models/scaling.pkl"),
        imputer_path=os.path.join(abs_path, "models/imputer.pkl"),
        categorical_vectorizer_path=os.path.join(abs_path, "models/dv_x.pkl"),
    ...
    type_model = model.get_clf()
    assert type_model[-1] == "sgd_" or type_model[-1] == "log_"


def test_fit_model(dict_class_params: dict, abs_path: str):
    params = ModelsParams(
        scaling_path=os.path.join(abs_path, "models/scaling.pkl"),
        imputer_path=os.path.join(abs_path, "models/imputer.pkl"),
        categorical_vectorizer_path=os.path.join(abs_path, "models/dv_x.pkl"),
